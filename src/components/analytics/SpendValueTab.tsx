    ...
    <AnalyticsLayout rail={<InsightsRail insights={insights} tab="spend_value" />}>
      ...
      <div className="rounded-2xl bg-card shadow-[0_2px_16px_-4px_hsl(var(--foreground)/0.06)] p-8">
        <div className="flex items-center gap-3 text-muted-foreground mb-3">
          ...
          <p className="text-sm font-medium font-analytics tracking-wide">Your Best Value This Month</p>
        </div>
        {bestValue.bestProteinItem ? (
          <div>
            <p className="text-3xl font-analytics font-light text-foreground tracking-tight">{bestValue.bestProteinItem}</p>
            <p className="text-base text-muted-foreground font-analytics mt-1">
              delivers <span className="font-semibold text-foreground">{bestValue.bestProteinValue.toFixed(1)}g protein</span> per AED — your most cost-efficient protein source.
            </p>
          </div>
        ) : (
      ...
      {storeChartData.length > 0 && (
        <AnalyticsModule title="Spend by Store" icon={Store}>
          <div className="h-44">
      ...
      <AnalyticsModule title="Store Breakdown (This Month)" icon={Store} variant="receipt">
        {spending.stores.length === 0 ? (
          ...
          <div className="font-analytics">
            {/* Receipt header */}
            <div className="flex justify-between text-[10px] uppercase tracking-widest text-muted-foreground pb-2 border-b border-dashed border-border">
              <span>Store</span>
            ...
            {spending.stores.map((s, i) => (
              ...
              >
                ...
                <div className="flex gap-6 shrink-0">
                  <span className="w-12 text-right text-muted-foreground tabular-nums">{s.visits}</span>
                  <span className="w-20 text-right text-foreground font-semibold tabular-nums">{formatCurrencyAlways(s.total)}</span>
                  <span className="w-20 text-right text-muted-foreground tabular-nums">{formatCurrencyAlways(s.avgPerVisit)}</span>
                </div>
              </div>
            ))}
            {/* Receipt total */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <AnalyticsModule title="Protein Efficiency" icon={BarChart3}>
            ...
            <div className="space-y-2.5 font-analytics">
              {bestValue.items.slice(0, 5).map((item, i) => (
                <div key={item.name} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="shrink-0 text-muted-foreground tabular-nums font-medium">{item.proteinPerAed.toFixed(1)}g/AED</span>
                </div>
              ))}
            </div>
          )}
